<?php
    ...
    $filter=[];
    if(isset($_GET['q'])) $filter['title']=$_GET['q'];
    if(isset($_GET['conf'])) $filter['conf']=$_GET['conf'];
    if(isset($_GET['year'])) $filter['year']=$_GET['year'];
    if(isset($_GET['journal'])) $filter['journal']=$_GET['journal'];
    $list=$paperModel->filter($filter);
?>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="cm-title-section mb-5">
                    <h1 style="text-align: center;">PAPERIZE<br><small style="font-size:1.5rem;color:rgba(0, 0, 0, .54);">Advanced Search</small></h1>
                </div>
                ...
                <?php if(!empty($list)){ ?>
                    <?php foreach($list as $l) Helper::display($l); ?>
                <?php } ?>
            </div>
        </div>
    </div>
